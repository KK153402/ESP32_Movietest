...
def convert_all():
    ...
    for file_path in files:
        ...
        out_audio  = os.path.join(OUTPUT_DIR, f"{clean_name}.mp3")
        ...
        cmd_audio = [
            ...
            '-vn', '-ac', '1', '-ar', '22050', '-ab', '128k',
            out_audio
        ]

        cmd_video = [
            FFMPEG_CMD, '-y', '-i', file_path,
            '-vf', 'scale=240:240:force_original_aspect_ratio=decrease:force_divisible_by=8,pad=240:240:(ow-iw)/2:(oh-ih)/2:black',
            '-c:v', 'mjpeg', '-q:v', '7',
            '-r', str(TARGET_FPS),
            '-an', temp_video
        ]
